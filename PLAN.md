...
4. Проверь событии там был максимум 3 событии в день и так они не случаются ли сразу то есть например в этот день 2 событии нужно быть и они не должны быть оба в одно время то есть сразу после первого второе событие в графике это не красиво видно сделай так чтобы в событии можно было поставить время даже автоматические ивенты должны быть в списке как ожидающие и с временами ! и создания событии сделай чтобы можно было поставить время !

5. На окне детальной информации которая появляется при нажатии на кнопку на карточке баланса там задай немного нижнего отступа потом сделай так чтобы close all orders был только когда есть ордеры а если нет то просто спрячь его 

6. Нужно придумать новые увлекательные возможности ! Моя первая идея чтобы заманивать больше людей ! сразу после регистрации любой может получить бонус в размере 10$ на свой реальный счет
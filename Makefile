...
api:
	HTTP_ADDR="$(HTTP_ADDR)" DB_DSN="$(DB_DSN)" JWT_ISSUER="$(JWT_ISSUER)" JWT_SECRET="$(JWT_SECRET)" JWT_TTL="$(JWT_TTL)" INTERNAL_API_TOKEN="$(INTERNAL_API_TOKEN)" WS_ORIGIN="$(WS_ORIGIN)" PROFECT_MODE="$(PROFECT_MODE)" TELEGRAM_BOT_TOKEN="$(TELEGRAM_BOT_TOKEN)" UI_DIST="$(UI_DIST)" MARKETDATA_DIR="$(MARKETDATA_DIR)" go run ./cmd/api

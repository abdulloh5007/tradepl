...
Можно ли сделать так чтобы логи про ошибках на терминале на vite выходил один раз то есть например на циклических функциях если сделать ошибку то лог переполняется и тут зависает программа код редактор потому что в терминале каждые миг попадает карсный некрасивый лог.

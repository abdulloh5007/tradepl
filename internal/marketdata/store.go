...
func (s *Store) GetPairBySymbol(ctx context.Context, symbol string) (Pair, error) {
	...
	err := s.pool.QueryRow(ctx, `
		select
			...
			min_qty, min_notional, contract_size, pnl_contract_size, lot_step, min_lot, max_lot, status
		from trading_pairs
		where symbol = $1
	`, symbol).Scan(
		&p.ID, &p.Symbol, &p.BaseAssetID, &p.QuoteAssetID, &p.PricePrecision, &p.QtyPrecision,
		&p.MinQty, &p.MinNotional, &p.ContractSize, &p.PnLContractSize, &p.LotStep, &p.MinLot, &p.MaxLot, &p.Status,
	)
	return p, err
}

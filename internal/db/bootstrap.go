...
// EnsureCoreReferenceData bootstraps minimal static data required by runtime:
// - USD/UZS assets
// - UZS-USD trading pair
// - default account plans
// It is idempotent and safe to call on every server startup.
func EnsureCoreReferenceData(ctx context.Context, pool *pgxpool.Pool) error {
	...
	// Backfill invalid/empty values without overriding valid custom settings.
	if _, err := tx.Exec(ctx, `
